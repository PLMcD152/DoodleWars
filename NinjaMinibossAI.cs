...
public class NinjaMinibossAI : MonoBehaviour {
	...
	private NinjaMinibossHealth eh;
    ...
    void Start ()
	{
		eh = GetComponent<NinjaMinibossHealth> ();
    ...
    void Patrol()
	{
		if (distance > 5 && target.transform.position.x > transform.position.x)
		{
			GetComponent<Rigidbody2D>().velocity = new Vector2 (speed, GetComponent<Rigidbody2D>().velocity.y);
			anim.SetBool("walking", true);
			
		}
		else if (distance > 5 && target.transform.position.x < transform.position.x)
		{
			GetComponent<Rigidbody2D>().velocity = new Vector2 (-speed, GetComponent<Rigidbody2D>().velocity.y);
			anim.SetBool("walking", true);
		}
